...
rules = []
with open('Day_7_input_data.txt') as f:
...
for rule in rules:
    ...
    parsed_contained_bags = [
        re.match('(\d)\s([\s\w]+)\s((bags)|(bag))', b) for b in contained_bags]
    parsed_contained_bags = [(int(m.group(1)), m.group(2))
                             for m in parsed_contained_bags if m]
